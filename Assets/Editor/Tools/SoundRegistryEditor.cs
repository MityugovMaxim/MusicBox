...
[CustomEditor(typeof(SoundRegistry), true)]
public class SoundRegistryEditor : Editor
{
	bool            m_Initialized;
	...
	void Initialize()
	{
		...
		SoundRegistry registry = target as SoundRegistry;
	...
	void FindAll()
	{
		...
		List<ScriptableObject> entries = AssetDatabase.FindAssets($"t:{nameof(SoundInfo)}")
		...
		SoundAttributeDrawer.Loaded = false;
	}
}