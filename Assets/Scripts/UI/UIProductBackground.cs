...
public class UIProductBackground : UIEntity
{
	...
	public async void Setup(string _ProductID, bool _Instant = false)
	{
		...
		try
		{
			Sprite sprite = await m_StorageProcessor.LoadProductBackground(_ProductID, token);
		...
		{
			Debug.Log("[UIProductPreviewBackground] Setup canceled.");
		}
		finally
		{
