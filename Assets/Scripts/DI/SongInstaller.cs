...
public class SongInstaller : MonoInstaller
{
	...
	ScopeConcreteIdArgConditionCopyNonLazyBinder InstallPool<TItem, TPool>(TItem _Prefab, int _InitialSize) where TItem : Object where TPool : IMemoryPool
	{
		return Container.BindMemoryPool<TItem, TPool>()
			.WithInitialSize(_InitialSize)
			.FromComponentInNewPrefab(_Prefab)
			.UnderTransformGroup($"[{typeof(TItem).Name}] Pool");
	}

	ScopeConcreteIdArgConditionCopyNonLazyBinder InstallPool<TItem, TPool>(TItem _Prefab, int _InitialSize, object _ID) where TItem : Object where TPool : IMemoryPool
	{
		return Container.BindMemoryPool<TItem, TPool>()
			.WithId(_ID)
			.WithInitialSize(_InitialSize)
