...
{
    private readonly float[] m_Ath;
    private readonly int[] m_Bark;
    private readonly int m_EighthOctaveLines;
    private readonly int m_FirstOctave;
    private readonly float m_MVal;
    private readonly int m_N;
    private readonly float[][] m_NoiseOffset;
    private readonly int[] m_Octave;
    private readonly PsyInfo m_PsyInfo;
    private readonly int m_ShiftOctave;
    private readonly List<OffsetMemory<float>> m_Sort;

    private readonly float[][][] m_ToneCurves;
    private readonly int m_TotalOctaveLines;

    public PsyLookup(PsyInfo _PsyInfo, PsyGlobal _GlobalParam, int _N, int _SampleRate)
    {
        m_PsyInfo = _PsyInfo;
        m_N = _N;
        m_Ath = new float[_N];
        m_Octave = new int[_N];
        m_Bark = new int[_N];
        m_Sort = new List<OffsetMemory<float>>(32);

        int lo = -99, hi = 1;

        m_EighthOctaveLines = _GlobalParam.EighthOctaveLines;
        m_ShiftOctave = (int)Math.Round(Math.Log(_GlobalParam.EighthOctaveLines * 8f) / Math.Log(2)) - 1;

        m_FirstOctave = (int)(ToOctave(.25f * _SampleRate * .5 / _N)
                              * (1 << (m_ShiftOctave + 1))
                              - _GlobalParam.EighthOctaveLines);

        var maxOctave = (int)(ToOctave((_N + .25f) * _SampleRate * .5 / _N)
                               * (1 << (m_ShiftOctave + 1))
                               + .5f);

        m_TotalOctaveLines = maxOctave - m_FirstOctave + 1;

        // AoTuV HF weighting 
        m_MVal = 1;
        if (_SampleRate < 26000)
            m_MVal = 0;
        else if (_SampleRate < 38000)
            m_MVal = .94f; // 32kHz 
        else if (_SampleRate > 46000)
            m_MVal = 1.275f; // 48kHz
        ...
        {
            var endPos = (int)Math.Round(FromOctave((k + 1) * .125 - 2) * 2.0 * _N / _SampleRate);
            var baseAth = m_AthSource[k];
            if (j < endPos)
            {
                var delta = (m_AthSource[k + 1] - baseAth) / (endPos - j);
                for (; (j < endPos) && (j < _N); j++)
                {
                    m_Ath[j] = baseAth + 100;
                    baseAth += delta;
        ...
        for (var i = 0; i < _N; i++)
        {
            var bark = ToBark(_SampleRate / (2 * _N) * i);

            for (;
                (lo + _PsyInfo.NoiseWindowLowMin < i)
                && (ToBark(_SampleRate / (2 * _N) * lo) < bark - _PsyInfo.NoiseWindowLow);
                lo++)
            {
            }

            for (;
                (hi <= _N) && ((hi < i + _PsyInfo.NoiseWindowHighMin)
                              || (ToBark(_SampleRate / (2 * _N) * hi) < bark + _PsyInfo.NoiseWindowHigh));
                hi++)
            {
            }

            m_Bark[i] = ((lo - 1) << 16) + (hi - 1);
        }

        for (var i = 0; i < _N; i++)
            m_Octave[i] = (int)(ToOctave((i + .25f) * .5 * _SampleRate / _N)
                                * (1 << (m_ShiftOctave + 1))
                                + .5f);

        m_ToneCurves = SetupToneCurves(
            _PsyInfo.ToneAtt,
            _SampleRate * .5 / _N,
            _N,
            _PsyInfo.ToneCenterBoost,
            _PsyInfo.ToneDecay);

        /* set up rolling noise median */
        m_NoiseOffset = new float[m_PsyInfo.NoiseOffset.Length][];
        for (var i = 0; i < m_NoiseOffset.Length; i++)
            m_NoiseOffset[i] = new float[_N];

        for (var i = 0; i < _N; i++)
        {
            var halfOctave = ToOctave((i + .5) * _SampleRate / (2.0 * _N)) * 2.0;
            ...
            for (var h = 0; h < m_NoiseOffset.Length; h++)
                m_NoiseOffset[h][i] = (float)(_PsyInfo.NoiseOffset[h][intHalfOctave] * (1 - del)
                                              + (del > 0 ? _PsyInfo.NoiseOffset[h][intHalfOctave + 1] * del : 0));
        }
    }

    private static float[][][] SetupToneCurves(
        float[] _CurveAttDecibels,
        double _BinHertz,
        int _N,
        float _CenterBoost,
        float _CenterDecayRate)
    {
        var bruteBuffer = new float[_N];
        ...
        for (var i = 0; i < PsyInfo.BANDS; i++)
        {
            ...
            for (var j = 0; j < EhmerMax; j++)
            {
                ...
                for (var k = 0; k < 4; k++)
                    ...
                    {
                        if (min > m_AthSource[j + k + athOffset])
                            min = m_AthSource[j + k + athOffset];
                    }
                    else
                    {
                        if (min > m_AthSource[MaxAth - 1])
                            min = m_AthSource[MaxAth - 1];
                    }
            ...
            for (var j = 0; j < 6; j++)
                workc[i][j + 2] = m_ToneMasks[i][j].ToArray().ToFixedLength(EhmerMax);

            workc[i][0] = m_ToneMasks[i][0].ToArray().ToFixedLength(EhmerMax);
            workc[i][1] = m_ToneMasks[i][0].ToArray().ToFixedLength(EhmerMax);

            /* apply centered curve boost/decay */
            for (var j = 0; j < Levels; j++)
                for (var k = 0; k < EhmerMax; k++)
                {
                    var adj = _CenterBoost + Math.Abs(EhmerOffset - k) * _CenterDecayRate;

                    if ((adj < 0) && (_CenterBoost > 0))
                        adj = 0;
            ...
            for (var j = 0; j < Levels; j++)
            {
                AttenuateCurve(
                    workc[i][j],
                    _CurveAttDecibels[i] + 100f - (j < 2 ? 2 : j) * 10f - Level0);
        ...
        for (var i = 0; i < PsyInfo.BANDS; i++)
        {
            ...
            var bin = (int)Math.Floor(FromOctave(i * .5) / _BinHertz);
            var lowCurve = (int)Math.Ceiling(ToOctave(bin * _BinHertz + 1) * 2);
            var highCurve = (int)Math.Floor(ToOctave((bin + 1) * _BinHertz) * 2);
            ...
            for (var m = 0; m < Levels; m++)
            {
                ...
                for (var k = lowCurve; k <= highCurve; k++)
                {
                    ...
                    for (var j = 0; j < EhmerMax; j++)
                    {
                        var lowBin = (int)(FromOctave(j * .125 + k * .5 - 2.0625) / _BinHertz);
                        var highBin = FromOctave(j * .125 + k * .5 - 1.9375) / _BinHertz + 1;
                ...
                if (i + 1 < PsyInfo.BANDS)
                {
                    ...
                    for (var j = 0; j < EhmerMax; j++)
                    {
                        var lowBin = (int)(FromOctave(j * .125 + i * .5 - 2.0625) / _BinHertz);
                        var highBin = (int)(FromOctave(j * .125 + i * .5 - 1.9375) / _BinHertz + 1);
                ...
                for (var j = 0; j < EhmerMax; j++)
                {
                    var bufferBin = (int)(FromOctave(j * .125 + i * .5 - 2.0) / _BinHertz);
                    if (bufferBin < 0)
                    {
    ...
    public void ToneMask(
        float[] _PCM,
        float[] _Logmask,
        float _GlobalSpecMax,
        float _LocalSpecMax)
    {
        float[] seedArr = null;
        Span<float> seed = m_TotalOctaveLines <= 128
            ? stackalloc float[m_TotalOctaveLines]
            : (seedArr = ArrayPool<float>.Shared.Rent(m_TotalOctaveLines));

        seed = seed.Slice(0, m_TotalOctaveLines);

        var att = _LocalSpecMax + m_PsyInfo.AthAdjAtt;

        for (var i = 0; i < seed.Length; i++)
            seed[i] = NegativeInfinite;

        // set the ATH (floating below localmax, not global max by a specified att)
        if (att < m_PsyInfo.AthMaxAtt)
            att = m_PsyInfo.AthMaxAtt;

        for (var i = 0; i < m_N; i++)
            _Logmask[i] = m_Ath[i] + att;

        // tone masking 
        SeedLoop(m_ToneCurves, _PCM, _Logmask, seed, _GlobalSpecMax);
        MaxSeeds(seed, _Logmask);
    ...
    public void OffsetAndMix(
        ...
        float[] _Logmask,
        float[] _Mdct,
        in Span<float> _Logmdct)
    {
        var toneAtt = m_PsyInfo.ToneMasterAtt[_OffsetIndex];

        double cx = m_MVal;

        for (var i = 0; i < m_N; i++)
        {
            var val = _Noise[i] + m_NoiseOffset[_OffsetIndex][i];

            if (val > m_PsyInfo.NoiseMaxSuppress)
                val = m_PsyInfo.NoiseMaxSuppress;

            _Logmask[i] = Math.Max(val, _Tone[i] + toneAtt);
            ...
            if (_OffsetIndex == 1)
            {
                ...
                val -= _Logmdct[i]; // val == mdct line value relative to floor in dB 
    ...
    public void NoiseMask(in Span<float> _Logmdct, float[] _Logmask)
    {
        var workArr = ArrayPool<float>.Shared.Rent(m_N);
        Span<float> work = new Span<float>(workArr, 0, m_N);

        BarkNoiseHybridMp(_Logmdct, _Logmask, 140, -1);

        for (var i = 0; i < m_N; i++)
            work[i] = _Logmdct[i] - _Logmask[i];

        BarkNoiseHybridMp(work, _Logmask, 0, m_PsyInfo.NoiseWindowFixed);

        for (var i = 0; i < m_N; i++)
            work[i] = _Logmdct[i] - work[i];

        for (var i = 0; i < m_N; i++)
        {
            var dB = (int)(_Logmask[i] + .5);

            if (dB >= m_PsyInfo.NoiseCompand.Length)
                dB = m_PsyInfo.NoiseCompand.Length - 1;

            if (dB < 0)
                dB = 0;

            _Logmask[i] = work[i] + m_PsyInfo.NoiseCompand[dB];
        }
    ...
    private void BarkNoiseHybridMp(
        ...
        int _Adjusted)
    {
        var arr = ArrayPool<float>.Shared.Rent(m_N * 5);
        Span<float> arrSpan = arr;

        var fp = arrSpan.Slice(0, m_N);
        var fx = arrSpan.Slice(m_N, m_N);
        var fxx = arrSpan.Slice(m_N * 2, m_N);
        var fy = arrSpan.Slice(m_N * 3, m_N);
        var fxy = arrSpan.Slice(m_N * 4, m_N);
        ...
        for (i = 0, x = 0f; ; i++, x += 1f)
        {
            ...
            if (r - _Offset < _Noise[i]) _Noise[i] = r - _Offset;
        }
        for (; ; i++, x += 1f)
        {
            hi = i + _Adjusted / 2;
    ...
    public void CoupleQuantizeNormalize(
        ...
        int _Channels)
    {
        var partition = m_PsyInfo.Normalize ? m_PsyInfo.NormalPartition : 1;
        var limit = _PsyGlobal.CouplingPointLimit[m_PsyInfo.BlockFlag][_Blobno];
        var prepoint = m_StereoThresholds[_PsyGlobal.CouplingPrePointAmp[_Blobno]];
        var postpoint = m_StereoThresholds[_PsyGlobal.CouplingPostPointAmp[_Blobno]];

        // non-zero flag working vector 
        Span<bool> nz = stackalloc bool[_Channels];

        // energy surplus/deficit tracking 
        Span<float> acc = stackalloc float[_Channels + _Mapping.CouplingSteps];

        // The threshold of a stereo is changed with the size of n 
        if (m_N > 1000)
            postpoint = m_StereoThresholdsLimit[_PsyGlobal.CouplingPostPointAmp[_Blobno]];

        // mdct is our raw mdct output, floor not removed. 
        // inout passes in the ifloor, passes back quantized result 

        var arr = ArrayPool<float>.Shared.Rent(_Channels * partition * 3);

        // unquantized energy (negative indicates amplitude has negative sign) 
        var raw = arr.AsSpan(0, _Channels * partition);

        // dual pupose; quantized energy (if flag set), othersize fabs(raw) 
        var quantMemory = arr.AsMemory(_Channels * partition, _Channels * partition);
        var quant = quantMemory.Span;

        // floor energy 
        var floor = arr.AsSpan(_Channels * partition * 2, _Channels * partition);

        // flags indicating raw/quantized status of elements in raw vector 
        var flagArr = ArrayPool<bool>.Shared.Rent(_Channels * partition);
        Span<bool> flag = flagArr.AsSpan(0, _Channels * partition);

        for (var i = 0; i < _Channels + _Mapping.CouplingSteps; i++)
            acc[i] = 0f;

        for (var i = 0; i < m_N; i += partition)
        {
            var jn = partition > m_N - i ? m_N - i : partition;
            int step, track = 0;

            // prefill 
            for (var channel = 0; channel < _Channels; channel++)
            {
                nz[channel] = _Nonzero[channel];
                if (nz[channel])
                {
                    for (var j = 0; j < jn; j++)
                        floor[channel * partition + j] = m_DecibelLookup[_Iwork[channel][i + j]];

                    FlagLossless(limit, prepoint, postpoint, _Mdct[channel], floor, flag, channel * partition, i, jn);

                    for (var j = 0; j < jn; j++)
                    {
                        var index = channel * partition + j;

                        quant[index] = raw[index] = _Mdct[channel][i + j] * _Mdct[channel][i + j];

                        if (_Mdct[channel][i + j] < 0f)
                            raw[index] *= -1f;
            ...
            // coupling 
            for (step = 0; step < _Mapping.CouplingSteps; step++)
            {
                var mag = _Mapping.CouplingMag[step];
                var ang = _Mapping.CouplingAng[step];
                    ...
                    for (var j = 0; j < jn; j++)
                    {
                        if (j < _SlidingLowpass - i)
                            if (flag[mag * partition + j] || flag[ang * partition + j])
                            {
                                ...
                                // couple iM/iA 
                                {
                                    var a = _Iwork[mag][i + j];
                                    var b = _Iwork[ang][i + j];

                                    if (Math.Abs(a) > Math.Abs(b))
                                    {
                                        _Iwork[ang][i + j] = a > 0 ? a - b : b - a;
                                    }
                                    else
                                    {
                                        _Iwork[ang][i + j] = b > 0 ? a - b : b - a;
                                        _Iwork[mag][i + j] = b;
                                    }

                                    // collapse two equivalent tuples to one 
                                    if (_Iwork[ang][i + j] >= Math.Abs(_Iwork[mag][i + j]) * 2)
                                    {
                                        _Iwork[ang][i + j] = -_Iwork[ang][i + j];
                                        _Iwork[mag][i + j] = -_Iwork[mag][i + j];
                                    }
                                }
                            }
                            else
        ...
        int[] _Output)
    {
        ...
        int j;
        var start = m_PsyInfo.Normalize ? m_PsyInfo.NormalStart - _I : _N;
        if (start > _N)
            start = _N;
        ...
        for (j = 0; j < start; j++)
        {
            if ((_Flags != null) && _Flags[_Offset + j])
                continue;

            // Lossless coupling already quantized.
            // Don't touch; requantizing based on
            // energy would be incorrect.
            var ve = q[_Offset + j] / _F[_Offset + j];
            if (_R[_Offset + j] < 0)
                _Output[_I + j] = (int)-Math.Round(Math.Sqrt(ve));
            else
                _Output[_I + j] = (int)Math.Round(Math.Sqrt(ve));
        }
        ...
        for (; j < _N; j++)
            if ((_Flags == null) || !_Flags[_Offset + j])
            {
                ...
                if ((ve < .25f) && ((_Flags == null) || (j >= _Limit - _I)))
                {
                    acc += ve;
                    m_Sort.Add(new OffsetMemory<float>(_QMemory, j)); // q is fabs(r) for unflagged element
                }
                else
                {
                    ...
                    q[_Offset + j] = _Output[_I + j] * _Output[_I + j] * _F[_Offset + j];
                }
            }

        if (m_Sort.Count > 0)
        {
            m_Sort.Sort(m_Comparer);

            // noise norm to do 
            for (j = 0; j < m_Sort.Count; j++)
            {
                var k = m_Sort[j].Offset;
                if (acc >= m_PsyInfo.NormalThreshold)
                {
                    _Output[_I + k] = (int)UnitNorm(_R[_Offset + k]);
                    acc -= 1f;
                    q[_Offset + k] = _F[_Offset + k];
                }
                else
                {
                    _Output[_I + k] = 0;
    ...
    private static void FlagLossless(
        int _Limit, float _PrePoint, float _PostPoint, float[] _Mdct,
        in Span<float> _Floor, in Span<bool> _Flag, int _Offset, int _I, int _Jn)
    {
        for (var j = 0; j < _Jn; j++)
        {
            var point = j >= _Limit - _I ? _PostPoint : _PrePoint;
            var r = Math.Abs(_Mdct[j + _I]) / _Floor[_Offset + j];
            _Flag[_Offset + j] = r >= point;
        }
    }

    private static float UnitNorm(float _X)
    {
        var i = BitConverter.ToUInt32(BitConverter.GetBytes(_X), 0);
        i = (i & 0x80000000U) | 0x3f800000U;
        return BitConverter.ToSingle(BitConverter.GetBytes(i), 0);
    }

    private void MaxSeeds(in Span<float> _Seed, float[] _Floor)
    {
        var linePosition = 0;

        SeedChase(_Seed); // for masking

        var pos = m_Octave[0] - m_FirstOctave - (m_EighthOctaveLines >> 1);

        float minV;
        while (linePosition + 1 < m_N)
        {
            minV = _Seed[pos];
            var end = ((m_Octave[linePosition] + m_Octave[linePosition + 1]) >> 1) - m_FirstOctave;

            if (minV > m_PsyInfo.ToneAbsLimit)
                minV = m_PsyInfo.ToneAbsLimit;

            while (pos + 1 <= end)
            {
                pos++;
                if (((_Seed[pos] > NegativeInfinite) && (_Seed[pos] < minV)) || (minV <= NegativeInfinite))
                    minV = _Seed[pos];
            }

            end = pos + m_FirstOctave;
            for (; (linePosition < m_N) && (m_Octave[linePosition] <= end); linePosition++)
                if (_Floor[linePosition] < minV)
                    _Floor[linePosition] = minV;
        }

        minV = _Seed[m_TotalOctaveLines - 1];
        for (; linePosition < m_N; linePosition++)
            if (_Floor[linePosition] < minV)
                _Floor[linePosition] = minV;
    }

    private void SeedChase(in Span<float> _Seeds)
    {
        var posStackArr = ArrayPool<int>.Shared.Rent(m_TotalOctaveLines);
        var posStack = new Span<int>(posStackArr, 0, m_TotalOctaveLines);

        var ampStackArr = ArrayPool<float>.Shared.Rent(m_TotalOctaveLines);
        var ampStack = new Span<float>(ampStackArr, 0, m_TotalOctaveLines);

        var stack = 0;
        var pos = 0;

        for (var i = 0; i < m_TotalOctaveLines; i++)
            if (stack < 2)
            {
                posStack[stack] = i;
                ampStack[stack++] = _Seeds[i];
            }
            else
            {
                while (true)
                    if (_Seeds[i] < ampStack[stack - 1])
                    {
                        posStack[stack] = i;
                        ampStack[stack++] = _Seeds[i];
                        break;
                    ...
                    {
                        if (i < posStack[stack - 1] + m_EighthOctaveLines)
                            if ((stack > 1) && (ampStack[stack - 1] <= ampStack[stack - 2]) &&
                                (i < posStack[stack - 2] + m_EighthOctaveLines))
                            {
        ...
        for (var i = 0; i < stack; i++)
        {
            ...
            if ((i < stack - 1) && (ampStack[i + 1] > ampStack[i]))
                ...
                endPosition = posStack[i] + m_EighthOctaveLines + 1;

            if (endPosition > m_TotalOctaveLines) endPosition = m_TotalOctaveLines;
            for (; pos < endPosition; pos++)
                _Seeds[pos] = ampStack[i];
        }
    ...
    private void SeedLoop(float[][][] _Curves, float[] _PCM, float[] _Floor, in Span<float> _Seeds, float _Specmax)
    {
        ...
        for (var i = 0; i < m_N; i++)
        {
            ...
            if (max + 6f > _Floor[i])
            {
                ...
                SeedCurve(
                    ...
                    _Curves[oc],
                    max,
                    m_Octave[i] - m_FirstOctave,
                    m_TotalOctaveLines,
                    m_EighthOctaveLines,
                    m_PsyInfo.MaxCurveDecibel - _Specmax);
            }
        }
    }

    private static void SeedCurve(in Span<float> _Seeds, float[][] _Curves, float _Amp, int _Oc, int _N, int _Linesper,
        float _DBOffset)
    {
        var choice = (int)((_Amp + _DBOffset - Level0) * .1f);
        choice = Math.Max(choice, 0);
        choice = Math.Min(choice, Levels - 1);
        var posts = _Curves[choice];

        var post1 = (int)posts[1];
        var seedIndex = (int)(_Oc + (posts[0] - EhmerOffset) * _Linesper - (_Linesper >> 1));

        for (var i = (int)posts[0]; i < post1; i++)
        {
            if (seedIndex > 0)
            {
                var lin = _Amp + posts[2 + i];

                if (_Seeds[seedIndex] < lin)
                    _Seeds[seedIndex] = lin;
            }
    ...
    private static void MinCurve(float[] _C1, float[] _C2)
    {
    ...
    private static void MaxCurve(float[] _C1, float[] _C2)
    {
        for (var i = 0; i < EhmerMax; i++)
            if (_C2[i] > _C1[i])
                _C1[i] = _C2[i];
    ...
    private static void AttenuateCurve(float[] _Curve, float _Att)
    {
        for (var i = 0; i < EhmerMax; i++)
            _Curve[i] += _Att;
    }

    private static double ToOctave(double _N) => Math.Log(_N) * 1.442695f - 5.965784f;

    private static double FromOctave(double _N) => Math.Exp((_N + 5.965784f) * .693147f);

    private static double ToBark(double _N) => 13.1f * Math.Atan(.00074f * _N) + 2.24f * Math.Atan(_N * _N * 1.85e-8f) + 1e-4f * _N;

    private class ApComparer : IComparer<OffsetMemory<float>>
    {
        public int Compare(OffsetMemory<float> _X, OffsetMemory<float> _Y)
        {
            var f1 = _X[0];
            var f2 = _Y[0];
            return (f1 < f2 ? 1 : 0)
    ...
    private static readonly float[] m_StereoThresholdsLimit = new[]
        {0, .5f, 1, 1.5f, 2, 2.5f, 4.5f, 8.5f, 9e10f};

    private static readonly float[] m_StereoThresholds = new[]
        {0, .5f, 1, 1.5f, 2.5f, 4.5f, 8.5f, 16.5f, 9e10f};

    private static readonly float[] m_DecibelLookup = new[]
    {
    ...
    private static readonly ApComparer m_Comparer = new ApComparer();
    ...
    private static readonly float[] m_AthSource =
    {
    ...
    private static readonly float[][][] m_ToneMasks =
    {
            /* 62.5 Hz */
            new[]
            {
