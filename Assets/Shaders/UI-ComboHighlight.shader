	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(const fragData IN) : SV_Target
			{
				...
				color.rgb *= lerp(_BackgroundPrimaryColor, _BackgroundSecondaryColor, spectrum);
				#else
				color.rgb *= lerp(_ForegroundPrimaryColor, _ForegroundSecondaryColor, spectrum);
				#endif
				
				
				color.a *= remap(spectrum, 0, 0.2, 0, 1);
