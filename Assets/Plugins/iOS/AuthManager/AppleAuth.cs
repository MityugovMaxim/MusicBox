...
public static class AppleAuth
{
	...
	static extern void AppleAuthManager_Login(AppleAuthSuccessCallback _Success, AppleAuthFailedCallback _Failed);
	#endif
	...
	public static Task<(string IDToken, string Nonce)> LoginAsync()
	{
		...
		#if UNITY_IOS && !UNITY_EDITOR
		AppleAuthManager_Login(InvokeLoginSuccess, InvokeLoginFailed);
		#else
